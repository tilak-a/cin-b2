...
const isAtBottom = scrollPosition + viewportHeight >= totalHeight - 50;
...
function detectCurrentSection() {
  ...
  const section5 = document.getElementById("s1");
  const section5Top = section5.offsetTop;
  const section5Bottom = section5Top + section5.offsetHeight;
  const totalHeight = document.documentElement.scrollHeight;
  const viewportHeight = window.innerHeight;
  const isAtBottom = scrollPosition + viewportHeight >= totalHeight - 150;
  ...
  sections.forEach((section, index) => {
    ...
    // Skip section detection if user is at the absolute bottom
    if (isAtBottom) {
      console.log("called return", currentSectionIndex);
  ...
  console.log("called this section", currentSectionIndex);
...
function selectPlanet(planetName) {
  ...
  if (mercuryLink) {
    ...
    setTimeout(() => {
      ...
      if (supportsHEVCAlpha()) {
        sunVideo.innerHTML = `
          <source src="assets/globs/sunoutputnew.mov">
        `;
      } else {
        sunVideo.innerHTML = `
          <source src="assets/globs/sun.webm" type='video/webm'>
        `;
      }
      ...
      setTimeout(() => {
        ...
        if (supportsHEVCAlpha()) {
          if (planet.includes("Earth")) {
            sunVideo.innerHTML = `
              <source src="assets/globs/sun.mov"'>
            `;
          } else {
            sunVideo.innerHTML = `
              <source src="assets/globs/sunoutputnew.mov">
...
function mySunFunc() {
  ...
  const is1020 = window.matchMedia(
    "(min-width: 1221px) and (max-width: 1223px)"
  ).matches;
  const isTablet = window.matchMedia(
    "(min-width: 520px) and (max-width: 1220px)"
  ).matches;
  ...
  if (csection == 1) {
    ...
    if (is1650) {
      ...
      if (window.innerHeight > 800) {
        bottomPosition = 12.512820512820511;
      } else {
    ...
    if (!ismobile) {
      if (isTablet) {
        if (window.innerWidth > 1000) {
          sunVideo.style.bottom = `${window.innerHeight / 12}px`;
        } else {
          sunVideo.style.bottom = `${bottomPosition}%`;
        }
        // sunVideo.style.top = `-${window.innerHeight / 5}px`;
      } else {
        sunVideo.style.bottom = `${bottomPosition}%`;
      }
    }
    if (ismobile) {
      sunVideo.style.bottom = `-${window.innerHeight / 14}px`;
      // sunVideo.style.bottom = `-10lvh`;
  ...
  if (csection == 2) {
    ...
    if (is1650) {
      ...
      if (window.innerHeight > 710) {
        ...
        bottomPosition = -28;
        scale = 0.5;
        sunVideo.style.width = "100%";
      } else {
        scale = 0.5;
        leftPosition = 50;
        bottomPosition = -40; // Adjusted higher for tablet
        sunVideo.style.width = "53%";
      }
    } else if (isTablet) {
    ...
    if (isTablet) {
      if (window.innerWidth > 1000) {
        if (window.innerHeight > 1300) {
          sunVideo.style.bottom = `-${window.innerHeight / 7}px`;
          sunVideo.style.width = `70%`;
        } else {
          sunVideo.style.bottom = `-${window.innerHeight / 2}px`;
          sunVideo.style.width = `70%`;
        }
      } else {
        sunVideo.style.bottom = `${bottomPosition}%`;
